# StudyForge — Detailed Implementation TODO Checklist

> Use this with the Stage-by-Stage Plan. A checkbox can be marked complete only after code path, state path, UI behavior, persistence, prompt visibility, error handling and manual test are verified.

---

## 0. Discovery and Baseline

- [ ] Identify real skeleton entry HTML and make a backup/versioned copy.
- [ ] List all existing screens, DOM IDs, CSS components, modals and event handlers.
- [ ] Identify actual Gemini text, image and grounding function names/signatures from integration examples.
- [ ] Identify current model names, API/key/config approach and response formats.
- [ ] Identify current state/localStorage keys and any direct DOM-only state.
- [ ] Identify working skeleton UI features not to alter.
- [ ] Create requirement-to-code audit: DONE/PARTIAL/NOT DONE for Documents 1–7.
- [ ] Identify and report conflicts/unknowns before coding.
...
- [ ] Implement technical extraction quality assessment.
...
## 5. Notes, Enrichment and Containers
...
## 6. Images, CA, Refinement and Dynamic Parts
