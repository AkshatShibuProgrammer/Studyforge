...
# Final Reconciliation Addendum — Conditional Enrichment and Runtime Dependencies

The maps above must include the following later-agreed dependency branch before per-part text generation:

```text
{part plan + source slice + part analysis + all definitions/formulas + blueprint + bundles}
     ↓
◇ Does this part have a selected and relevant deferred enrichment feature?
     ├─ Location central/supporting → CTX-LOC → contextual enrichment or grounded-CA query
     ├─ Person central/supporting → CTX-PER → PSC-relevant enrichment or grounded-CA query
     └─ Formula Operations applies → CTX-FORM → formula operations pack
     ↓
AI-07A structured notes generation
```

## Additional dependency rules

| Event | Required dependency behavior |
|---|---|
| User edits source | invalidates all analysis, parts, blueprints and generated output; canonical source must be re-established |
| User changes global bundle | invalidates only assignments/blueprints/generation affected by that feature, not deep extraction |
| User changes blueprint image item | invalidates target image prompt/asset, not unrelated text |
| User refreshes CA | marks affected validation result stale; source-backed content remains intact |
| Context subcall needs current fact | it returns a CA-query requirement; it must not silently ground itself |
| Prompt user-edit regeneration | saved final prompt includes canonical system prompt + user edit + REG-02 change context |
| Any response fails schema | no state mutation; controlled format repair then normal retry path |

## Extended final generation chain

```text
AI-07A safe blocks
  → planned/writer/post-review/validation/highlight image request
  → normalize + known-anchor validation + reserved slot
  → AI-07B prompt expansion
  → optional prompt preview user gate
  → AI-07C asset
  → attach image; validation becomes stale if asset changes

AI-07D grounded CA
  → URL/date/publisher/fetch time validation
  → WEB_SOURCED only
  → insert/update CA block
  → part validation becomes stale
```

This addendum does not remove any earlier dependency map. It adds the later-agreed branches and safety gates.
