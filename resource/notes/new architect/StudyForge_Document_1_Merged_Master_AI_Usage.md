...
# Final Reconciliation Addendum — Later Agreed Prompt-Architecture Details

This addendum is part of the merged Document 1 and preserves later decisions that must be read together with every preceding master table, stage table, inventory and appendix.

## Added conditional Stage 7 context-enrichment operations

| ID | Exact use | Trigger | Context / cache key | Grounding | Required result |
|---|---|---|---|---|---|
| `CTX-LOC` | Context-specific facts for a location in the current part | Location is central/supporting **and** Geography/Location feature applies | `location + context hash + source version` | OFF; request AI-07D CA if current facts needed | relevant facts only, map need, CA query suggestion |
| `CTX-PER` | PSC/exam-relevant facts for a source-mentioned person | Person is central/supporting **and** Personality feature applies | `person + exam + topic context + source version` | OFF; request AI-07D CA if current news needed | contribution/role/related people/exam tip only |
| `CTX-FORM` | Formula-operations pack | Formula feature applies to formula assigned to current part | `formula ID + feature set + source version` | OFF | variables, relationships, derivation context, safe illustrative example, edge cases/mistakes when relevant |

These can run as separate cached subcalls before `AI-07A`, or their exact canonical instructions may be embedded in the saved final `AI-07A` prompt. They are never generic fact-dump calls.

## Later-agreed runtime additions

1. Every prompt has model settings: model, temperature, top-p, output budget, grounding state, schema, retry class, prompt version and actual model used.
2. `AI-03A` additionally returns concise **content purpose**.
3. `AI-03B` additionally extracts source-supported stable `KF-*` key facts/statistics/rankings.
4. All routed bundle fragments are stored individually: Geography, Personality, Formula, Data, Legal, Timeline, Story, Administration, Process, CS, Medical/Science and CA relevance.
5. CA and any current person/location fact must use AI-07D grounded search; normal writing/enrichment must never claim unverified latest data.
6. Prompt execution history stores final system instruction, final task prompt, input references, model, grounding, token estimates, retry/model-switch state, response ref and error state.
7. Raw AI HTML is not directly inserted: responses are schema-validated, container type/anchor validated and rendered through application allowlists.

## Cross-document precedence

- Document 3 is the exact stored prompt text.
- Document 4 is the exact runtime/caching/retry/settings contract.
- This document remains the complete master AI usage explanation and inventory.
